...
{
  y::function<bool(const y::string&, y::size,
                   const y::string&, y::size)> check_pattern_recursive = [&](
  ...
  auto check_pattern = [&](const y::string& path, const y::string& pattern)
  {
  ...
  if (cpattern.find("**", star_pos) < right) {
    ...
    y::copy_if(
        ...
        [&](const y::string& p){return check_pattern(p, cpattern);});
    return;
  }
                                   ...
                                   const y::string& path) const
{
  auto process_segment = [&](
      y::vector<y::string>& split, const y::string& segment)
  {
    if (segment == "..") {
      if (split.empty()) {
        return false;
